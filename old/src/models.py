...
class entity:
    ...
    def updateConstants(self, board, time):
        # Update the internally constants and return whether the animal is alive
        self.board = board
        self.time = time
        self.energy -= (self.maxEnergy / 10)
        self.health -= self.maxEnergy - self.energy 
        if self.health <= 0:
        ...
        if self.currentGoal != None:
            if self.currentGoal.type == "move":
                ...
                if distance < self.speed: # If the animal can move to the location in one turn
                    ...
                    self.energy -= round(distance * (2-self.efficiency))
                    if type(self.board[currentMoveTarget[0]][currentMoveTarget[1]]) == foodSource: # If the animal moved to a food source
                        self.energy += self.board[currentMoveTarget[0]][currentMoveTarget[1]].energy
                    ...
                    for i in range(math.floor(self.speed)):
                        ...
                        if smallestEndDistance < distance:
                            break
                    self.energy -= round(smallestEndDistance * (2-self.efficiency))
